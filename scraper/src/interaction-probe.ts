...
// Kept back from action timeouts so the after-measurement still fits.
...
// Allowlist, not blocklist: a button with an unknown label is never clicked.
...
// Runs on capturePage's open page after the DOM is captured; a failed probe leaves interactions null.
...
// Backstop for a page that hangs past the time budget.
...
async function runProbe(page: Page): Promise<InteractionCapture> {
  ...
  // Fires only for a new document, so a pushState load-more is not leaving the page.
  const onNewDocument = () => {
  ...
  try {
    // Zero clicks, before anything changes the page.
...
async function measureChars(page: Page, state: ProbeState): Promise<number> {
  // A control that navigated makes every later measurement describe another page.
...
async function charsOutsideBanner(page: Page, state: ProbeState): Promise<number> {
  ...
  // Accepting may only hide the banner, so subtract text still in the DOM.
...
// Runs inside the page: must not use anything from module scope.
...
function locateInPage(options: {
  ...
  if (options.kind === "consent") {
    // Document order: the first match is the outermost banner.
